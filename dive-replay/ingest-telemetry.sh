...
# Create mapping
curl -XPUT 'localhost:9200/telemetry' -H 'Content-Type: application/json' -d'
  {
    "mappings": {
      "_record": {
        "properties": {
...
FILES=`find /opt/openrov/data`
for FILE in $FILES; do
  curl -XPOST 'http://localhost:9200/telemetry/_record/' -H 'Content-Type: application/json' -d @${FILE}
done
